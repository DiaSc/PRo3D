   ...
   // open Newtonsoft.Json
    ...
    let view<'ma> 
        ...
        if usePackedAnnotationRendering then
            ...
            let lines, pickIds, bb = PackedRendering.linesNoIndirect config.offset hoveredAnnotation (model.annotations.selectedLeaves |> ASet.map (fun e -> e.id)) (model.annotations.hoveredLeaves) (annoSet |> ASet.map ((fun (g, (s,t)) -> g,s))) viewMatrix
